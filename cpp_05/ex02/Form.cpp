...
Form::Form(std::string const &name, int gradeForSigning, int gradeForExecute):
	name(name), gradeForSigning(gradeForSigning), gradeForExecute(gradeForExecute), sign(false)
{
	checkGrade();
}
...
Form::Form(Form const &form):
	...
	gradeForExecute(form.getGradeForExecute()), sign(form.getSign())
{
	checkGrade();
}
...
bool Form::checkExe(Bureaucrat const &br) const
{
	if (br.getGrade() > this->gradeForExecute)
		throw Form::GradeTooLowerException();
	else if (!this->sign)
		return (false);
	return (true);
